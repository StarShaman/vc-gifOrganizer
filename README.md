...
- **Bookmarks sidebar**: right-click a category card → *Bookmark category* → pick a built-in icon (with a color of your choice) or upload your own image. Bookmarked categories appear in a sidebar on the picker's left edge (with a Favorites shortcut on top) for one-click jumps. Deleting a category removes its bookmark.
